// LovyanGFX CollisionCircles — ported to M5Unified
// Original by lovyan03: https://github.com/lovyan03/LovyanGFX
// Physics-based bouncing balls with elastic collisions
...
{
  static constexpr float e = 0.999;
  ...
  for (int i = 0; i != (int)ball_count; i++) {
    a = &balls[i];
    ...
    } else if (a->x >= (int32_t)_width - a->r) {
      a->x = _width - a->r - 1;
      if (a->dx > 0) a->dx = -a->dx * e;
    }
    a->y += a->dy;
    ...
    } else if (a->y >= (int32_t)_height - a->r) {
      a->y = _height - a->r - 1;
      if (a->dy > 0) a->dy = -a->dy * e;
    }
    for (int j = i + 1; j != (int)ball_count; j++) {
      b = &balls[j];
